...
if [[ "${SCAMPER_SKIP_SMOKE:-0}" == "1" ]]; then
  ...
  if [[ ",${SCAMPER_MEASUREMENTS:-trace,rr}," == *",trace6,"* ]]; then
    run_scamper_smoke_test gcp "$TRACE_ARGS" 6
  fi
fi
...
if [[ -n "${SCAMPER_TRACE6_TARGET_COUNT:-}" ]]; then
  campaign_args+=(--trace6-target-count "$SCAMPER_TRACE6_TARGET_COUNT")
fi
if [[ -n "${SCAMPER_TRACE6_TARGET_SHA256:-}" ]]; then
  campaign_args+=(--trace6-target-sha256 "$SCAMPER_TRACE6_TARGET_SHA256")
fi
campaign_args+=(
  --checkpoint-command
